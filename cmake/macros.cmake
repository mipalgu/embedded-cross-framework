...
macro(build_project_for_boards project boards)
    foreach(board ${boards})
        ...
        if(DEFINED ${subproject}_EXECUTABLE_SUFFIX)
            set(CMAKE_EXECUTABLE_SUFFIX ${${subproject}_EXECUTABLE_SUFFIX})
        elseif(DEFINED ${subproject}_EXECUTABLE_SUFFIX)
            set(CMAKE_EXECUTABLE_SUFFIX ${${subproject}_EXECUTABLE_SUFFIX})
        elseif(DEFINED ${TARGET_TRIPLET}_EXECUTABLE_SUFFIX)
            set(CMAKE_EXECUTABLE_SUFFIX ${${TARGET_TRIPLET}_EXECUTABLE_SUFFIX})
        elseif(DEFINED ${TARGET_TRIPLET}_USE_FPGA)
            set(CMAKE_EXECUTABLE_SUFFIX .bit)
        else()
            set(CMAKE_EXECUTABLE_SUFFIX .elf)
        endif()
        set(executable ${subproject})
...
macro(create_build_scripts_for_board project board subproject executable)
    message(STATUS "Building FPGA project ${subproject} for board ${board} in project ${project} creating executable ${executable}${CMAKE_EXECUTABLE_SUFFIX}.")
...
macro(build_subproject_for_board project board subproject executable)
    ...
    if(INSTALL_SCRIPT)
        if(INSTALL_SCRIPT_ARGS)
            install(CODE "execute_process(COMMAND ${INSTALL_SCRIPT} ${INSTALL_SCRIPT_ARGS} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        else()
            install(CODE "execute_process(COMMAND ${INSTALL_SCRIPT} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        endif()
    elseif(${${subproject}_INSTALL_SCRIPT})
        if(${${subproject}_INSTALL_SCRIPT_ARGS})
            install(CODE "execute_process(COMMAND ${${subproject}_INSTALL_SCRIPT} ${${subproject}_INSTALL_SCRIPT_ARGS} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        else()
            install(CODE "execute_process(COMMAND ${${subproject}_INSTALL_SCRIPT} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        endif()
    elseif(${${project}_INSTALL_SCRIPT})
        if(${${project}_INSTALL_SCRIPT_ARGS})
            install(CODE "execute_process(COMMAND ${${project}_INSTALL_SCRIPT} ${${project}_INSTALL_SCRIPT_ARGS} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        else()
            install(CODE "execute_process(COMMAND ${${project}_INSTALL_SCRIPT} ${CMAKE_CURRENT_BINARY_DIR}/${executable}${CMAKE_EXECUTABLE_SUFFIX})")
        endif()
