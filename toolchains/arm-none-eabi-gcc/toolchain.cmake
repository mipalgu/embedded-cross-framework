...
find_program(TOOLCHAIN_OBJCOPY "${TARGET_TRIPLET}-objcopy${TOOLCHAIN_EXE}")
MESSAGE(STATUS "TOOLCHAIN_OBJCOPY: " ${TOOLCHAIN_OBJCOPY})
find_program(TOOLCHAIN_OBJDUMP "${TARGET_TRIPLET}-objdump${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_RANLIB "${TARGET_TRIPLET}-ranlib${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_READELF "${TARGET_TRIPLET}-readelf${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_SIZE "${TARGET_TRIPLET}-size${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_STRIP "${TARGET_TRIPLET}-strip${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_AR "${TARGET_TRIPLET}-ar${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_NM "${TARGET_TRIPLET}-nm${TOOLCHAIN_EXE}")
find_program(TOOLCHAIN_LINKER "${TARGET_TRIPLET}-ld${TOOLCHAIN_EXE}")
...
if (EXISTS ${TOOLCHAIN_OBJCOPY})
    set(CMAKE_OBJCOPY ${TOOLCHAIN_OBJCOPY} CACHE INTERNAL "objcopy tool")
else()
    set(CMAKE_OBJCOPY ${TOOLCHAIN_PREFIX}-objcopy${TOOLCHAIN_EXE} CACHE INTERNAL "objcopy tool")
endif()
if (EXISTS ${TOOLCHAIN_OBJDUMP})
    set(CMAKE_OBJDUMP ${TOOLCHAIN_OBJDUMP} CACHE INTERNAL "objdump tool")
else()
    set(CMAKE_OBJDUMP ${TOOLCHAIN_PREFIX}-objdump${TOOLCHAIN_EXE} CACHE INTERNAL "objdump tool")
endif()
if (EXISTS ${TOOLCHAIN_RANLIB})
    set(CMAKE_RANLIB ${TOOLCHAIN_RANLIB} CACHE INTERNAL "ranlib tool")
else()
    set(CMAKE_RANLIB ${TOOLCHAIN_PREFIX}-ranlib${TOOLCHAIN_EXE} CACHE INTERNAL "ranlib tool")
endif()
if (EXISTS ${TOOLCHAIN_READELF})
    set(CMAKE_READELF ${TOOLCHAIN_READELF} CACHE INTERNAL "readelf tool")
else()
    set(CMAKE_READELF ${TOOLCHAIN_PREFIX}-readelf${TOOLCHAIN_EXE} CACHE INTERNAL "readelf tool")
endif()
if (EXISTS ${TOOLCHAIN_SIZE})
    set(CMAKE_SIZE ${TOOLCHAIN_SIZE} CACHE INTERNAL "size tool")
else()
    set(CMAKE_SIZE ${TOOLCHAIN_PREFIX}-size${TOOLCHAIN_EXE} CACHE INTERNAL "size tool")
endif()
if (EXISTS ${TOOLCHAIN_STRIP})
    set(CMAKE_STRIP ${TOOLCHAIN_STRIP} CACHE INTERNAL "strip tool")
else()
    set(CMAKE_STRIP ${TOOLCHAIN_PREFIX}-strip${TOOLCHAIN_EXE} CACHE INTERNAL "strip tool")
endif()
if (EXISTS ${TOOLCHAIN_AR})
    set(CMAKE_AR ${TOOLCHAIN_AR} CACHE INTERNAL "ar tool")
else()
    set(CMAKE_AR ${TOOLCHAIN_PREFIX}-ar${TOOLCHAIN_EXE} CACHE INTERNAL "ar tool")
endif()
if (EXISTS ${TOOLCHAIN_NM})
    set(CMAKE_NM ${TOOLCHAIN_NM} CACHE INTERNAL "nm tool")
else()
    set(CMAKE_NM ${TOOLCHAIN_PREFIX}-nm${TOOLCHAIN_EXE} CACHE INTERNAL "nm tool")
endif()
if (EXISTS ${TOOLCHAIN_LINKER})
    set(CMAKE_LINKER ${TOOLCHAIN_LINKER} CACHE INTERNAL "ld tool")
else()
...
set(ARM_CPU_ABI_SOFTFP_FLOAT_FLAGS "-mfloat-abi=softfp")
